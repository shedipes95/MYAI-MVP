...
import {
  runHomeInsuranceScraper,
  type HomeInsuranceResponse,
  type HomeInsuranceQuote,
} from "@/api/homeInsurance";
import homeUserData from "@/data/home_user_data.json";
...
export default function HomeInsurance() {
  ...
  const handleGetQuotes = async () => {
    ...
    try {
      ...
      const response: HomeInsuranceResponse = await runHomeInsuranceScraper(homeUserData);
      ...
      if (response.quotes && Array.isArray(response.quotes)) {
        ...
        const isMockData = response.quotes.some(
          (q) =>
            ...
            q.features &&
            q.features.includes("24/7 Claims helpline"),
        );
      ...
      console.error("Home insurance API error:", err);
    }
  };
  ...
  const getSavingsAmount = (quote: HomeInsuranceQuote) => {
                ...
                <table className="w-full">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="text-center p-4 font-semibold text-slate-700">Rating</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sortedQuotes.map((quote, index) => (
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-6">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-4 py-2 rounded-full text-sm font-medium">
              <span>🔍</span>
              <span>Smart Insurance Comparison</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              Home Insurance Quotes
            </h1>
            <p className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed">
              Compare personalized quotes from Ireland's leading insurance providers and
              <span className="font-semibold text-green-600"> save up to €200+ per year</span>
            </p>
          </div>
        ...
        {/* Enhanced Property Summary Card */}
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-slate-800">Your Property</h3>
                  <p className="text-slate-600">Dublin 8 • Semi-detached home</p>
                </div>
              </div>
              <div className="text-right">
                <div className="text-sm text-slate-500">Eircode</div>
                <div className="font-mono font-bold text-slate-800">
                  {homeUserData.address_eircode}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <div className="text-center space-y-2">
                ...
                <div>
                  ...
                  <div className="font-bold text-slate-800">{homeUserData.bedrooms}</div>
                </div>
              </div>
              ...
              <div className="text-center space-y-2">
                ...
                <div>
                  <div className="text-sm text-slate-500">Rebuilding Cost</div>
                  <div className="font-bold text-slate-800">
                    {formatCurrency(Number(homeUserData.rebuilding_cost))}
                  </div>
                </div>
              </div>
              ...
              <div className="text-center space-y-2">
                ...
                <div>
                  <div className="text-sm text-slate-500">Contents Cover</div>
                  <div className="font-bold text-slate-800">
                    {formatCurrency(Number(homeUserData.contents_cover))}
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-600">
                    Built in {homeUserData.year_building_constructed}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-green-500">✓</span>
                  <span className="text-slate-600">{homeUserData.heating_system} heating</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-green-500">✓</span>
                  <span className="text-slate-600">Monitored alarm</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-green-500">✓</span>
                  <span className="text-slate-600">3 years claims free</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Takes 30 seconds • 5+ providers • No commitment required
          </p>
        </div>
        ...
        {state === "loading" && (
          <Card className="text-center py-12 rounded-2xl shadow-sm">
            <div className="space-y-4">
              ...
              <p className="text-lg text-slate-600">Searching for the best quotes...</p>
        ...
        <Card className="shadow-sm rounded-2xl">
          <div className="text-center space-y-4">
            <h2 className="text-xl font-semibold text-slate-800">Why Compare with MyAI?</h2>
